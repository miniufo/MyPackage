 ...
 * @since   MDK1.0
 */
public final class LagrangianStatisticsByDavisCC extends SingleParticleStatistics{
	
	/**
	 * constructor, the lengths of Particles in the list should be the same
	 */
	public LagrangianStatisticsByDavisCC(List<? extends Particle> ls,DataDescriptor dd){ super(ls,dd);}
		...
		void computeMean(Predicate<Record> cond){
			...
			for(Particle p:ls){
				...
				for(int l=0,L=p.getTCount();l<L;l++){
					...
					if(vspd[l]==Record.undef) continue;
					
					//umn+=uspd[l];
					vmn+=vspd[l]; pseudoTracks++;
					
					for(int ll=0;ll<L;ll++){
						int tau=ll-l;
						
						if(tau>tRad||tau<-tRad) continue;
						if(vspd[ll]==Record.undef) continue;
						...
						if(llpos){
							...
							av.addSample(tau,0,vspd[ll],0,disY);
							
						}else{
							av.addSample(tau,0,vspd[ll],0,p.getYPosition(ll)-oY);
						}
					}
				}
			}
			...
			for(Particle p:ls){
				...
				for(int l=0,L=p.getTCount();l<L;l++){
					...
					for(int ll=0;ll<L;ll++){
						...
						av.addSample(tau,0,Pyy,0,0,0,va);
					}
				}
			}
				...
				for(int l=0,L=p.getTCount();l<L;l++){
					...
					for(int ll=0;ll<L;ll++){
						...
						if(llpos){
							...
							av.addSample(-tau,0,Kyy,0,0);
							av2.addSample(tau,0,disY);
